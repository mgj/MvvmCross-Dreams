...
namespace Dreams.Core.Tests.Services.DreamsFetcher
{
    [TestFixture]
    public class DreamsFetcherServiceTests : MvxTestFixtureBase
    {
        ...
        {
            var sut = Mvx.Resolve<IDreamsFetcherService>() as DreamsFetcherServiceMock;
            sut.FetchFromWebResponse = FetcherResponseValidFactory();

            var response = await sut.Fetch(new Uri(DreamsSettings.DREAMSWEBSERVICE_BASE_URL));
